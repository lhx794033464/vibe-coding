...
4. **连接线路由规则**：
   ...
   - 连接线只能是水平或垂直线段，不允许斜线
   - **判断节点定义**：当一个节点A延伸出多个子节点（如B、C、D）时，A即为判断节点
   - **判断节点多分支连线规则（重要）**：
     * 判断节点有多个子节点时，必须从该节点的不同侧面延伸连线到各个子节点
     * **三个子节点（纵向布局）**：从判断节点的左侧、下侧、右侧分别延伸三条连线，连接到三个子节点，三条线呈倒T字形对称分布
     * **三个子节点（横向布局）**：从判断节点的上侧、左侧、下侧分别延伸三条连线，连接到三个子节点，三条线呈├字形对称分布
     * **两个子节点**：从判断节点的左侧和右侧（纵向布局）或上侧和下侧（横向布局）分别延伸连线，保持对称
     * **严禁**所有连线从判断节点的同一个侧面延伸，避免线条重叠

...
function parseMermaidToDrawIO(mermaidCode: string, direction: 'vertical' | 'horizontal'): string {
  ...
  const isHorizontal = firstLine.toUpperCase().includes('LR') || firstLine.toUpperCase().includes('RL');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // 解析连接: A --> B 或 A -->|"label"| B 或 A -->|label| B
    // 支持 A --> B 在同一行定义节点: A(["start"]) --> B["process"]
    const edgeMatch = line.match(/([\w\(\[\{][\w\s\[\]\(\)\{\}"]*)\s*(-->\||-->)\s*(?:\|"?([^|]+)"?\|)?\s*([\w\(\[\{][\w\s\[\]\(\)\{\}"]*)/);
    if (edgeMatch) {
      const [, fromPart, arrowType, label, toPart] = edgeMatch;
      ...
      const extractNode = (part: string) => {
        ...
        if (defMatch) {
          ...
          if (!nodes.has(id)) {
            nodes.set(id, { text, type, x: startX, y: startY });
          }
          return id;
        }
        // 纯 ID 无定义
  ...
  nodes.forEach((node, id) => {
    ...
    const width = node.type === 'decision' ? 100 : (node.type === 'start' || node.type === 'end' ? 120 : 160);

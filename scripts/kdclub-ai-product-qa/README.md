...
1. 将 `kdclub-ai-product-qa` 整个目录复制到对应 Agent 的 `skills/` 目录下。
2. 重启工具使技能生效。
...
```bash
# QoderWork
cp -r kdclub-ai-product-qa ~/.qoderwork/skills/

# WorkBuddy
cp -r kdclub-ai-product-qa ~/.workbuddy/skills/

# Claude Code
cp -r kdclub-ai-product-qa ~/.claude/skills/

# OpenClaw
cp -r kdclub-ai-product-qa ~/.openclaw/skills/
```
